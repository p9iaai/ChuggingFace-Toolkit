...
def generate_image_with_timeout(model, prompt, negative_prompt, guidance_scale, num_inference_steps, width, height, seed, timeout):
    """
...
def generate_image(prompt, model="stable-diffusion-3.5-large", negative_prompt="blurry, bad quality", guidance_scale=7.5, num_inference_steps=40, width=1024, height=1024, max_retries=9):
    """
    ...
    - model (str): The model to use: stable-diffusion-3.5-large, stable-diffusion-3.5-large-turbo, stable-diffusion-3.5-medium, stable-diffusion-2, stable-diffusion-2-1 or stable-diffusion-xl-base-1.0
    ...
    # Save the image
    output_dir = f"output/stable-diffusion/{model}"
    ...
    with open(f".logs/stable-diffusion.txt", "a") as log_file:
